...
def evaluate(opt, args):
    ...
    for dataset in datasets:
        ...
        gt_root = os.path.join(opt.Eval.gt_root, dataset)

        preds = os.listdir(pred_root)

        gts = os.listdir(gt_root+"/masks")
        ...
        for i, sample in samples:
            pred, gt = sample
            # print(pred, gt)
            # print(pred, gt ,os.path.splitext(pred)[0] ,os.path.splitext(gt)[0])
            assert os.path.splitext(pred)[0] == os.path.splitext(gt)[0]


            pred_mask = np.array(Image.open(os.path.join(pred_root, pred)))

            gt_mask = np.array(Image.open(os.path.join(gt_root+"/masks", gt)))
