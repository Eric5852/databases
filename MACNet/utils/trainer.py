...
def adjust_lr(optimizer, epoch, decay_rate=0.1, decay_epoch=50):
    if epoch <=decay_epoch:
        decay = decay_rate ** int(epoch / decay_epoch)
        for param_group in optimizer.param_groups:
            param_group['lr'] *= decay
    elif   decay_epoch<=epoch<=1.5*decay_epoch:
        decay = decay_rate
        if epoch==decay_epoch:
            for param_group in optimizer.param_groups:
                param_group['lr'] *= decay
    else:
        decay=decay_rate ** (epoch // (decay_epoch+9))
        for param_group in optimizer.param_groups:
            param_group['lr'] *= decay
